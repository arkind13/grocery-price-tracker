...
# ---------------------------------------------------------------------------
# Section C: Sheet I/O (headless connection)
# ---------------------------------------------------------------------------
...
def _find_root_env() -> Path:
    """Walk up from this file to find the workspace .env. Max 6 levels."""
...
def _build_credentials():
    ...
    Raises RuntimeError with secret-free message on failure.
    """
...
def _connect_worksheet(worksheet_name: str = "Products_Master"):
    """Open the worksheet by key (GROCERY_SPREADSHEET_ID) via gspread.

    Returns the gspread Worksheet object.
    Raises RuntimeError if env vars missing or connection fails.
    """
    import gspread
