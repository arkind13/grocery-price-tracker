...
| `lists` | `[--full]` | **THE 6 USER-FACING LISTS — LIVE counts (2026-09-03).** Lists 1-3 + 6 from one fresh sheet read (Unmatched = debt minus ignored minus keyword-exists; wool/coles missing = keyword cols I/J with `NA` counting as populated); list 4 = the merged to-do queue; list 5 = ignored file; list 6 = **Missed pricing** (FIXABLE: keyword present + price unusable + not GONE; DELETE-PENDING: both stores unusable, GONE included). Missing-list counts annotate `(N pending website adds)` (handshake, not dupes) and `(N also in missed pricing)` for cross-list rows. `--full` prints names with ⏳/⚠ markers. NEVER counts the Wednesday `.txt` snapshots (the root cause of the 2026-09-03 stale-counts incident). Sheet failure → "unavailable" + verbatim error, exit 0 |
| `todo` | `show` / `done --items "1,HUY,SRM"` / `gone --items "1,HUY"` | **The ONE website-add queue (2026-09-03 user rule: no gaps).** to-do (price written, keyword pending) + searched (new row) MERGED — continuous numbering, Coles then Woolworths, `[CODE]` per entry, `· new row` marks searched entries. `done` (merged numbers and/or codes, all-or-nothing — both queues validated before either mutates) removes entries AND writes the remembered exact store name as the row's store keyword for BOTH kinds (closes the "searched has no done" gap). **`gone` (2026-09-03 user rule):** the item is verified unavailable at that store — the keyword is LEFT ALONE, the literal `GONE` is written into the store's price cell (D/E), and the entry is removed from the queue (all-or-nothing, same selection as `done`). Wednesday Step 1b list-presence auto-drain remains the evidence-based exit for searched entries |
| `missed-pricing` | `[--purge]` `[--dry-run]` | List #6 on demand: FIXABLE + DELETE-PENDING with store, keyword, cell text and fail age. **GONE word (2026-09-03):** type `GONE` into a price cell (D/E) = verified unavailable → never captured again, never stomped by markers (a returning real price clears it). `--purge` deletes ALL delete-pending rows NOW (explicit user action; archived to `data/deleted_rows.json`, ledger strikes cleared); `--dry-run` previews |
| `add-to-list` | `show` / `done` | **ALIASES of the merged `todo` view (2026-09-03)** — same numbering, same done semantics (always writes the keyword). Numbers are MERGED-view positions |
| `searched-items` | `show` / `remove` / `clear` | `show` = ALIAS of the merged `todo` view; `remove --items "KAT"` / `clear` still operate by code with 7-day tombstones |
...
| `wednesday` | `[--source docx]` (live RETIRED 2026-09-02 — refused at dispatch; see `lostbattle.md`) `[--dry-run]` `[--no-scp]` `[--no-telegram]` `[--no-prompt]` | Full pipeline. **Step 0 (both sources, 2026-09-02):** pull VPS queues → tombstone-aware union-merge (nothing lost on either side) → push back identical → print the queues. **Docx pause:** on a real terminal the run WAITS — add the queued items to the store website lists, re-paste the docx lists, type `done` (auto-skips with no TTY / `--no-prompt`). Then: parse lists → **Step 1b auto-clears queued items that now appear on the parsed lists** → match/sync prices → **Step 3b two-strike dead-row auto-delete (2026-09-03):** both-dead rows (neither price usable, GONE included) are deleted only when a PREVIOUS run already saw them dead (`data/delete_candidates.json`; same-day rerun never deletes; recovered rows leave the ledger); every deletion archived to `data/deleted_rows.json` → resolve lists + scp → Telegram **six-list** post (unmatched, wool/coles missing, **missed pricing** — FIXABLE one-store failures + DELETE-PENDING both-dead rows, to-do+searched merged, forgotten) → specials report → **Step 9** mirrors queues back to the VPS. **Docx is the ONLY live source now** — the live window (login + flush + fetch) was retired after the lost store-bot war; the manual website adds during the pause replace the flush |
...
> **Live lists + no gaps (2026-09-03):** all user-facing counts come from `lists` — one fresh sheet read + live queue files, never the Wednesday `.txt` snapshots (root cause of the resolved-but-still-45/10 incident). The to-do and searched queues are ONE merged queue (`todo`); every price-without-keyword path leaves a reminder there (including `search --add-item` merges onto keyword-less rows), and `done` ALWAYS writes the keyword. Missed pricing captures every keyword-present price failure (one-store included) with two exits: fix the keyword or the manual `GONE` price-cell verdict; both-dead rows enter the guarded two-strike deletion pipeline instead. Legacy commands (`add-to-list`, `searched-items`, `no-price`) are aliases of the new views so any instruction set reports the same truth. All state lives in host bind mounts on the VPS — container restarts/recreates change nothing.
