"""T8 round-B2 FINAL battery: fair R2-11 probe (fresh connection), D16 adds,
R2-3/D22 variants, exact dups, subsets, teardown.

Sheet state at entry: grid 218 = content bottom 218 (full), 104 contiguous
ZZT8R rows. A fresh connect gives an ACCURATE cached .rows — the production
condition. The very next add (row 219) must make add_product_row's guard
expand the grid itself (no raw APIError) — R2-11's fair live test.
(The previous crash — stale cached .rows after an out-of-band metadata
shrink — is recorded separately as R19.)
"""
import csv
import json
import sys
...
fails = []
ws = connect_worksheet()   # fresh: cached .rows == actual grid
print(f"cached ws.row_count at connect: {ws.row_count}", flush=True)
...
for n in range(start_n, start_n + 10):
    ...
    if res.get("wrote") and res.get("row_index"):
        ...
        if n == start_n:
            gr_after_first = grid_rows_of(ws)
            fired = gr_after_first > gr_before
            log_op("R2-11", f"first add over full grid ({gr_before})",
                   "auto-expand", f"grid>{gr_before}",
                   "expanded, no raw APIError",
                   f"grid={gr_after_first}", fired)
            print(f"R2-11 FAIR PROBE: grid {gr_before} -> {gr_after_first} "
                  f"(guard {'FIRED' if fired else 'DID NOT FIRE'})",
                  flush=True)
    else:
        refused += 1
...
log_op("A", "bulk verify", "adds", "all",
       f"on-sheet=={len(added)}, 0 phantom",
       f"phantom={ph}", ph == 0)
print(f"bulk verify: total={len(added)} phantom={ph}", flush=True)

# ---- Phase B: 15 variants incl. exact D22 ----
...
for k, (ri, r) in enumerate(real_rows[:16]):
    ...
    if res.get("merged") and res.get("row_index"):
        ...
        notmerged_n += 1
        log_op("B", variant[:45], f"variant({k % 3}){' **D22**' if is_d22 else ''}",
               st, "EXPECTED merge",
...
(LOGDIR / "t8r2_summary.json").write_text(json.dumps({
    "mode": "final (fair R2-11 probe + remaining phases)",
    "adds_new": written, "adds_refused": refused, "phantom": ph,
    "synth_total": len(added),
    "r2_11_fair_probe": {"grid_before": gr_before,
                         "guard_fired": gr_after_first > gr_before,
                         "grid_after_first_add": gr_after_first,
                         "restored_to": 380},
