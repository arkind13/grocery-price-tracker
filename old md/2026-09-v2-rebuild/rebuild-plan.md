...
| 5 | Non-halal twin line (M-items) + speed budget + regression + FINAL TIDY | 0.5 d | ✅ **DONE (criterion 1 OPEN — user-gated)** — twin line live on ALL three meat query shapes (`also at Woolworths (non-halal): $13.54 — Woolworths Beef Mince 500g` [GJZ]), suite 605 green / 0 skipped (6 new twin tests incl. the mandatory beef-mince fixture), budgets price 5.1s / live 17.2s / batch 4.8s / list 4.5s ALL MET, real MERJ inbox ingest (vision → `Halal Lamb Curry` merged idempotently into [ZWX]) + parity audit ALIGNED before AND after, docs v2-rewritten + rebuild artifacts archived to `old md/2026-09-v2-rebuild/`, tracker+parent pushed, VPS md5-verified; **the timed live Wednesday fire remains USER-GATED** (fresh Woolworths docx not pasted this session — one command away: `grocery_price_cli.py wednesday`) | 2026-09-10 |
